...
module.exports = function (db) {
  router.get("/", isLoggedIn, async function (req, res, next) {
    ...
    try {
      ...
      query += ` ORDER BY GREATEST(di_buat, di_edit) DESC LIMIT ${total_row_displayed} OFFSET ${row_number};`;
  ...
  router.get("/home", isLoggedIn, async function (req, res, next) {
    try {
      let sql =
        "SELECT id_properti, kategori, kota, provinsi, jenis_properti, total_harga, judul, kamar_tidur, kamar_mandi, luas_properti, di_buat, di_edit, foto_produk FROM properti ORDER BY GREATEST(di_buat, di_edit) DESC LIMIT 15 OFFSET 0";
